...
export default function SettingsScreen() {
  ...
  const handleReorderLifeAreas = async ({ data }: { data: Array<LifeArea & { depth: number }> }) => {
    try {
      console.log('[Settings iOS] Drag ended, reordering life areas, count:', data.length);
      ...
      console.log('[Settings iOS] Sending reorder request with IDs:', lifeAreaIds);
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          I'll fix the drag-and-drop functionality for Life Areas. The issue is that the iOS version doesn't have the necessary dependencies and wrapper components. Let me implement a proper solution using `react-native-draggable-flatlist` which is already installed.

<write file="app/(tabs)/settings.ios.tsx">
import React, { useState, useEffect, useMemo } from 'react';
import {
  View,
  Text,
...
export default function SettingsScreen() {
  ...
  const loadCurrencyBalances = async () => {
    ...
    try {
      ...
      console.log('[Reports iOS] Filtered currency balances:', {
  ...
  const handleReorderLifeAreas = async (reorderedAreas: LifeArea[]) => {
    try {
      console.log('[Settings iOS] Reordering life areas, count:', reorderedAreas.length);
