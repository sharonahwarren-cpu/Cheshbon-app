...
export default function ReflectScreen() {
  ...
  useEffect(() => {
    console.log('[Reflect iOS] Component mounted');
    if (params.date) {
      const dateFromParam = new Date(params.date);
      if (!isNaN(dateFromParam.getTime())) {
        console.log('[Reflect iOS] Setting date from params:', dateFromParam);
  ...
  const loadData = async () => {
    console.log('[Reflect iOS] Loading data for date:', selectedDate.toISOString().split('T')[0]);
    setLoading(true);
    try {
      ...
      console.error('[Reflect iOS] Error loading data:', error);
  ...
  const openAddReflectionModal = () => {
    console.log('[Reflect iOS] Opening Add Reflection modal from + button');
  ...
  const openEditReflectionModal = (reflection: Reflection) => {
    console.log('[Reflect iOS] Opening Edit Reflection modal for:', reflection.id);
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      <AddReflectionModal
        ...
        onClose={() => {
          console.log('[Reflect iOS] Closing AddReflectionModal without saving');
          setShowAddReflectionModal(false);
          setEditingReflection(null);
          setPrefilledGoalId(undefined);
        }}
