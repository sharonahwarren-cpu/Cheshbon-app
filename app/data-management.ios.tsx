...
export default function DataManagementScreen() {
  ...
  const handleDownloadData = async () => {
    ...
    try {
      ...
      console.log('✅ [DATA MANAGEMENT iOS] File downloaded, status:', downloadResult.status);

      if (downloadResult.status !== 200) {
        throw new Error(`Download failed with status: ${downloadResult.status}`);
      }

      console.log('✅ [DATA MANAGEMENT iOS] File saved to:', downloadResult.uri);
      ...
      console.error('❌ [DATA MANAGEMENT iOS] Download error:', error);
  ...
  const confirmDeleteData = async () => {
    console.log('🗑️ [DATA MANAGEMENT iOS] Confirming delete all data...');
    setShowDeleteDataModal(false);
    setIsDeletingData(true);

    try {
      await authenticatedDelete('/api/user/data');
      console.log('✅ [DATA MANAGEMENT iOS] All data deleted successfully');
      setModalMessage('All your data has been permanently deleted. Your account remains active.');
      setShowSuccessModal(true);
    } catch (error: any) {
      console.error('❌ [DATA MANAGEMENT iOS] Delete data error:', error);
  ...
  const confirmDeleteAccount = async () => {
    console.log('🗑️ [DATA MANAGEMENT iOS] Confirming delete account...');
    setShowDeleteAccountModal(false);
    setIsDeletingAccount(true);

    try {
      await authenticatedDelete('/api/user/account');
      console.log('✅ [DATA MANAGEMENT iOS] Account deleted successfully');
      ...
      console.error('❌ [DATA MANAGEMENT iOS] Delete account error:', error);
