...
export default function SettingsScreen() {
  ...
  const getInitialSection = useCallback((): SettingsSection => {
    ...
    console.log('[Settings iOS] getInitialSection - params.section:', section);
  ...
  useEffect(() => {
    ...
    console.log('[Settings iOS] Setting currentSection to:', section);
    setCurrentSection(section);
  }, [params.section, getInitialSection]);

  // Load data on mount
  useEffect(() => {
    console.log('[Settings iOS] Component mounted');
    loadData(false);
  }, []);

  // Handle back button press
  const handleBackPress = useCallback(() => {
    console.log('[Settings iOS] Back button pressed, currentSection:', currentSection, 'params.section:', params.section);
  ...
  useFocusEffect(
    useCallback(() => {
      console.log('[Settings iOS] Screen focused, registering back handler');
      const backHandler = BackHandler.addEventListener('hardwareBackPress', handleBackPress);
      
      return () => {
        console.log('[Settings iOS] Screen unfocused, removing back handler');
        backHandler.remove();
      };
    }, [handleBackPress])
  );
  ...
  const loadData = async (isRefreshing: boolean = false) => {
    ...
    try {
      console.log('[Settings iOS] Loading data...');
      ...
      console.log('[Settings iOS] Data loaded successfully');
    } catch (error) {
      console.error('[Settings iOS] Error loading data:', error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const showError = (message: string) => {
    console.error('[Settings iOS] Error:', message);
  };

  const showSuccess = (message: string) => {
    console.log('[Settings iOS] Success:', message);
  };
